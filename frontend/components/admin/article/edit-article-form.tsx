  ...
  useEffect(() => {
    if (tinymceReady && typeof window !== "undefined" && (window as any).tinymce) {
      ...
      (window as any).tinymce.init({
        ...
        height: 500,
        menubar: true,
        license_key: "gpl", // set GPL license
        plugins: [
        ...
        setup: (editor: any) => {
          editor.on('init', () => {
            if (defaultValues?.content) {
              editor.setContent(defaultValues.content);
            }
          });
        }
      });
    }
  }, [tinymceReady, defaultValues]);
    ...
    <div className="bg-white rounded-xl">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div>
          ...
          />
          {defaultValues?.highlight_image && (
            <img
              src={`${process.env.NEXT_PUBLIC_STORAGE_URL}/${defaultValues.highlight_image}`}
              alt="Highlight lama"
        ...
        <div className="flex justify-end">
          <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-md shadow transition px-6 py-2">
            Simpan Artikel
          </Button>
        </div>
      </form>
