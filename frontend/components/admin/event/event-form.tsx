...
import CustomFileInputEvent from './custom-file-input-event';
...
export default function EventForm({
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-md w-full">
      <h1 className="text-2xl font-bold mb-4">Tambah Data Event</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div>
          ...
          <input {...register('name')} placeholder="Contoh: Festival Budaya Jubung"
            className="w-full border rounded-md px-3 py-2 shadow-sm" required />
        </div>

        <div>
          <label className="block mb-1 font-medium">Kategori</label>
          <input {...register('category')} placeholder="Contoh: Art Exhibition"
            className="w-full border rounded-md px-3 py-2 shadow-sm" />
        </div>

        <div>
          <label className="block mb-1 font-medium">Lokasi</label>
          <input {...register('location')} placeholder="Contoh: Gedung Seni Budaya Jember"
            className="w-full border rounded-md px-3 py-2 shadow-sm" />
        </div>

        <div>
          <label className="block mb-1 font-medium">Deskripsi Singkat</label>
          <input {...register('description')} placeholder="Contoh: Pameran seni kontemporer..."
            className="w-full border rounded-md px-3 py-2 shadow-sm" />
        </div>

        <div>
          <label className="block mb-1 font-medium">Embed Link Google Maps</label>
          <input {...register('embed_map_url')} placeholder="https://www.google.com/maps/embed?..."
            className="w-full border rounded-md px-3 py-2 shadow-sm" />
        </div>

        {embedLink && embedLink.trim() !== '' && (
          <div className="mt-4">
            <iframe src={embedLink} width="100%" height="300" className="rounded-md border"
        ...
        <div>
          ...
          <Controller name="image" control={control}
            render={({ field }) => <CustomFileInputEvent field={field} accept="image/*" supportFile="PNG, JPG, WEBP" />} />
        </div>

        {[1, 2, 3].map((num) => (
          <div key={num}>
            <label className="block mb-1 font-medium">{`Gallery Image ${num}`}</label>
            <Controller name={`gallery_image_${num}` as keyof EventFormValues} control={control}
              render={({ field }) => <CustomFileInputEvent field={field} accept="image/*" supportFile="PNG, JPG, WEBP" />} />
          </div>
        ))}

        <div className="flex gap-3 mt-4">
          <button type="submit" className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-md shadow">Simpan</button>
          <button type="button" onClick={() => reset()} className="flex-1 border px-4 py-2 rounded-md shadow">Reset</button>
        </div>
      </form>
    </div>
  );
